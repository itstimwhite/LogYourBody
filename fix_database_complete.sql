...
BEGIN
  ...
  INSERT INTO email_subscriptions (user_id, email)
...
SELECT 
    'Tables created: ' || string_agg(table_name, ', ' ORDER BY table_name) as status
FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name IN ('profiles', 'user_settings', 'body_metrics', 'subscriptions', 'email_subscriptions');

-- Check RLS is enabled
SELECT 
    'RLS enabled on: ' || string_agg(tablename, ', ' ORDER BY tablename) as status
FROM pg_tables 
WHERE schemaname = 'public' 
