...
EXECUTE format
('ALTER TABLE auth.users ALTER COLUMN instance_id SET DEFAULT %L::uuid', correct_instance_id);
...
CREATE OR REPLACE FUNCTION public.setup_user_data
(p_user_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.setup_user_data
(uuid) TO postgres, service_role, authenticated;
...
BEGIN
    ...
    VALUES
        (
            ...
            'authenticated',
            'authenticated',
            'minimal_test_' || extract(epoch from now())
    ::text || '@test.com',
            NOW
    (),
            NOW
...
BEGIN
    ...
    INSERT INTO public.profiles
        (id, email, name, gender, height)
    VALUES
        (v_user_id, p_email, 'Test User', 'male', 175);
    ...
    INSERT INTO public.subscriptions
        (user_id, status, trial_start_date, trial_end_date)
    VALUES
        (v_user_id, 'trial', CURRENT_DATE, CURRENT_DATE + 3);

    RETURN QUERY
    SELECT v_user_id, 'User created (auth bypassed)'
    ::text;
