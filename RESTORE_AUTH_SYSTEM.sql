...
DROP FUNCTION IF EXISTS public.manual_user_setup
(text);
DROP FUNCTION IF EXISTS public.complete_user_setup
(text);
DROP FUNCTION IF EXISTS public.setup_user_data
(uuid);
DROP FUNCTION IF EXISTS public.create_test_user
(text, text);
DROP FUNCTION IF EXISTS public.create_mock_user
(text, text);
...
IF user_phone IS NOT NULL AND user_phone != '' THEN
            user_email := regexp_replace
(user_phone, '[^0-9]', '', 'g') || '@sms.logyourbody.com';
        ELSE
...
BEGIN
    ...
    ::text,
            COALESCE
    ((NEW.raw_user_meta_data->>'height')::integer, 175),
            COALESCE
    (NEW.created_at, NOW
    ()),
            COALESCE
...
BEGIN
    ...
    ::text,
            COALESCE
    (NEW.created_at::date, CURRENT_DATE),
            COALESCE
    (NEW.created_at::date, CURRENT_DATE) + INTERVAL '3 days',
            COALESCE
    (NEW.created_at, NOW
    ()),
            COALESCE
