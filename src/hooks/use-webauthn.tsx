...
export function useWebAuthn(): WebAuthnState & WebAuthnActions {
  ...
  const loadUserCredentials = async () => {
    ...
    try {
      ...
      const storedCredentials = localStorage.getItem(`webauthn_credentials_${user.id}`);
  ...
  const registerBiometric = async (): Promise<boolean> => {
    ...
    try {
      ...
      localStorage.setItem(`webauthn_credentials_${user.id}`, JSON.stringify(updatedCredentials));
  ...
  const authenticateWithBiometric = async (): Promise<boolean> => {
    if (!state.isSupported || !state.isAvailable || !state.isRegistered || !user) {
      setState(prev => ({ ...prev, error: 'Biometric authentication not available' }));
      return false;
    }
    ...
    try {
      ...
      const matchingCredential = state.credentials.find(cred => 
        cred.id === assertion.id || cred.id === assertionId
      );
  ...
  const removeBiometric = async (credentialId: string): Promise<boolean> => {
    try {
      const updatedCredentials = state.credentials.filter(cred => cred.id !== credentialId);
      ...
      setState(prev => ({ ...prev, error: 'Failed to remove biometric credential' }));
      return false;
    }
  };
