...
export const methodSchema = z.object({
  value: z.enum(['scale', 'dexa', 'calipers', 'visual', 'bioimpedance', 'other']),
...
export const weightUtils = {
  ...
  convertWeight: (weight: WeightData, targetUnit: 'lbs' | 'kg'): WeightData => {
    ...
    const convertedValue = targetUnit === 'kg' 
  ...
  getWeightHelper: (weight: WeightData): string => {
    const converted = weightUtils.convertWeight(weight, weight.unit === 'lbs' ? 'kg' : 'lbs');
...
export const bodyFatUtils = {
  ...
  getPresets: (): Array<{ label: string; value: number; description: string }> => [
