...
export async function checkDatabaseStatus(): Promise<DatabaseStatus> {
  ...
  try {
    ...
    const profilesTableExists =
      !connectionError || connectionError.code !== "PGRST301";
    ...
    const userSettingsTableExists =
      !settingsError || settingsError.code !== "PGRST301";
...
export function logDatabaseStatus(status: DatabaseStatus) {
  ...
  console.log(
    "User settings table:",
    status.userSettingsTableExists ? "✅" : "❌",
  );
  console.log(
    "Email subscriptions table:",
    status.emailSubscriptionsTableExists ? "✅" : "❌",
  );
  if (status.error) {
