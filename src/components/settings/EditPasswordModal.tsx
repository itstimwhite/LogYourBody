...
export const EditPasswordModal = React.memo<EditPasswordModalProps>(function EditPasswordModal({
  isOpen,
  onClose,
  onSave,
}) {
  ...
  const PasswordInput = ({ 
    ...
    const showField = field === 'current' ? 'current' : field === 'newPassword' ? 'new' : 'confirm';
    
    return (
      <div className="space-y-2">
        <label className={tw.inputLabel}>
          {label}
        </label>
        <div className="relative">
          <input
            ...
            className={cn(
              tw.input,
              'pr-12',
              errors[field] && 'border-destructive focus:border-destructive focus:ring-destructive'
            )}
            disabled={isLoading}
        ...
        <div className={cn(tw.helperText, 'space-y-1')}>
          ...
          <div className="space-y-1 text-xs">
            <div className={cn(
              ...
              formData.newPassword.length >= 8 ? 'text-green-400' : 'text-muted-foreground'
            )}>
              <div className={cn(
                'h-1.5 w-1.5 rounded-full',
                formData.newPassword.length >= 8 ? 'bg-green-400' : 'bg-muted-foreground'
              )} />
              At least 8 characters
            </div>
            <div className={cn(
              'flex items-center gap-2',
              /[A-Z]/.test(formData.newPassword) ? 'text-green-400' : 'text-muted-foreground'
            )}>
              <div className={cn(
                'h-1.5 w-1.5 rounded-full',
                /[A-Z]/.test(formData.newPassword) ? 'bg-green-400' : 'bg-muted-foreground'
              )} />
            ...
            <div className={cn(
              'flex items-center gap-2',
              /\d/.test(formData.newPassword) ? 'text-green-400' : 'text-muted-foreground'
            )}>
              <div className={cn(
                'h-1.5 w-1.5 rounded-full',
                /\d/.test(formData.newPassword) ? 'bg-green-400' : 'bg-muted-foreground'
              )} />
              One number
            </div>
          </div>
