...
const PickerColumn = React.memo<{
  ...
  const handleDragEnd = (
    ...
    const newIndex = Math.max(
      0,
      Math.min(column.values.length - 1, column.selectedIndex - itemsOffset),
    );
  ...
  return (
    <div className="relative max-w-[120px] flex-1">
      {/* Label */}
      {column.label && (
        <div className={cn(tw.helperText, "mb-2 text-center")}>
          {column.label}
        </div>
      )}
      ...
      >
        {/* Selection indicator */}
        <div
          className="pointer-events-none absolute left-0 right-0 z-10 border-b border-t border-primary/30 bg-primary/5"
          style={{
        ...
        <motion.div
          ...
          transition={
            isDragging ? { duration: 0 } : settingsTokens.animation.normal
          }
        >
          {column.values.map((value, index) => (
            <motion.button
              ...
              className={cn(
                "flex w-full items-center justify-center text-center transition-all duration-200",
...
export const DatePickerWheel = React.memo<{
  ...
  const [selectedYear, setSelectedYear] = useState(
    years.indexOf(date.getFullYear().toString()),
  );
