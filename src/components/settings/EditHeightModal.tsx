...
export const EditHeightModal = React.memo<EditHeightModalProps>(function EditHeightModal({
  isOpen,
  onClose,
  currentHeight,
  units,
  ...
  useEffect(() => {
    ...
    if (selectedHeight < minHeightCm) {
      setError(`Height must be at least ${units === 'imperial' ? "2'0\"" : '61 cm'}`);
    } else if (selectedHeight > maxHeightCm) {
      setError(`Height must be less than ${units === 'imperial' ? "8'2\"" : '250 cm'}`);
    } else {
      setError(null);
    }
    ...
    >
      <div className="space-y-6">
        ...
        <div className={cn(tw.helperText, 'text-center space-y-1')}>
          ...
          <p>• {units === 'imperial' ? 'Minimum height: 2\'0"' : 'Minimum height: 61 cm'}</p>
