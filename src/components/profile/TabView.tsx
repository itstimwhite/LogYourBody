...
export const TabView = React.memo<TabViewProps>(function TabView({
  ...
  const handleSwipeStart = (event: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
    if (!swipeEnabled) return;
    ...
    <div className={cn('h-full flex flex-col', className)}>
      <Tab.Group selectedIndex={selectedIndex} onChange={handleTabChange}>
        ...
        <div className="flex-shrink-0 pt-safe-top">
          <Tab.List className="flex justify-center p-1 m-4 bg-secondary/20 rounded-xl">
            {tabs.map((tab, index) => (
              <Tab
                key={tab.id}
                className={({ selected }) =>
                  cn(
                    'relative flex-1 px-6 py-3 text-sm font-medium rounded-lg transition-all duration-200',
              ...
              {tabs.map((tab, index) => (
                ...
                >
                  <motion.div
                    ...
                    initial={{ 
                      opacity: 0, 
                      x: index > selectedIndex ? 20 : index < selectedIndex ? -20 : 0 
                    }}
                    animate={{ 
                      opacity: 1, 
                      x: 0,
                    ...
                    whileDrag={{ 
                      scale: 0.98,
                      rotateY: dragDirection === 'left' ? -2 : dragDirection === 'right' ? 2 : 0,
                    }}
                  >
                    {tab.content}
                  </motion.div>
      ...
      {swipeEnabled && dragDirection && (
        ...
        >
          <div className="bg-black/60 text-white px-3 py-1 rounded-full text-xs font-medium backdrop-blur-sm">
            {dragDirection === 'left' && selectedIndex < tabs.length - 1 && 'Swipe to next →'}
            {dragDirection === 'right' && selectedIndex > 0 && '← Swipe to previous'}
          </div>
        </motion.div>
      )}
    </div>
