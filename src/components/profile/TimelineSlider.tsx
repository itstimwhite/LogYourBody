...
export const TimelineSlider = React.memo<TimelineSliderProps>(function TimelineSlider({
  metrics,
  selectedIndex,
  onIndexChange,
  className,
  ...
  const selectedMetric = useMemo(() => metrics[state.selectedIndex], [metrics, state.selectedIndex]);
  ...
  const sliderProps = {
    ...
    'aria-valuetext': currentDate ? `${formatDateLong(currentDate)}, entry ${state.selectedIndex + 1} of ${metrics.length}` : undefined,
  ...
  return (
    <motion.div
      className={cn(
        'w-full bg-background border-t border-border/50 shadow-sm',
        tw.safeArea,
        className
      )}
...
export const useTimelineAnalytics = (metrics: BodyMetrics[], selectedIndex: number) => {
  useEffect(() => {
...
export const useTimelineAccessibility = (metrics: BodyMetrics[], selectedIndex: number) => {
