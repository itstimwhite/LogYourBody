-- Final comprehensive fix for user creation issues
-- This ensures users can be created through Supabase GUI and all auth methods
...
-- Step 3: Create the updated handle_new_user function with comprehensive error handling
...
BEGIN
    ...
    ON CONFLICT (id) DO UPDATE SET
        ...
        updated_at = NOW()
    WHERE profiles.email IS NULL;  -- Only update if email was missing
    ...
    ON CONFLICT (user_id) DO UPDATE SET
        updated_at = NOW()
    WHERE subscriptions.status IS NULL;  -- Only update if status was missing
...
INSERT INTO public.profiles (id, email, name, gender, height, created_at, updated_at)
SELECT 
    u.id,
...
INSERT INTO public.subscriptions (user_id, status, trial_start_date, trial_end_date, created_at, updated_at)
SELECT 
    u.id,
    'trial',
    COALESCE(u.created_at, NOW()),
