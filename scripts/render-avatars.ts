...
class AvatarRenderer {
  ...
  constructor() {
    ...
    // Add missing DOM methods to canvas for Three.js compatibility
    (canvas as any).addEventListener = () => {};
    (canvas as any).removeEventListener = () => {};
    (canvas as any).dispatchEvent = () => {};
    (canvas as any).style = {};
    (canvas as any).width = 1080;
    (canvas as any).height = 1080;
    (canvas as any).clientWidth = 1080;
    (canvas as any).clientHeight = 1080;
    (canvas as any).getContext = () => context;

    // Mock additional WebGL 2.0 functions for compatibility
    if (!context.texImage3D) {
      context.texImage3D = () => {};
    }
    if (!context.texSubImage3D) {
  ...
  async renderAvatar(params: AvatarParams): Promise<Buffer> {
    ...
    if (!baseMesh) {
      throw new Error(
        `${params.sex === "m" ? "Male" : "Female"} mesh not loaded`,
      );
    }
  ...
  async renderAllCombinations(): Promise<void> {
            ...
            for (const stature of statures) {
              ...
              try {
                ...
                if (count % 100 === 0) {
                  ...
                  console.log(
                    `Rendered ${count}/${totalCombinations} (${((count / totalCombinations) * 100).toFixed(1)}%) - ETA: ${Math.round(remaining)}s`,
                  );
                }
              } catch (error) {
                console.error(
    ...
    console.log(
      `✅ Completed rendering ${count} avatars in ${totalTime.toFixed(1)}s`,
    );
  }
