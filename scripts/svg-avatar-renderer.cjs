...
class SVGAvatarRenderer {
  ...
  calculateMorphTargets(params) {
    const muscleMorph = Math.max(
      0,
      Math.min(1, (params.ffmi - 14) / (25 - 14)),
    );
  ...
  createTorsoPath(params, morphs) {
    ...
    const points = [
      // Right shoulder
      {
        x: this.centerX + shoulderWidth / 2,
        y: this.centerY - torsoHeight / 2,
      },
      // Right waist
  ...
  createWireframeSVG(params) {
    ...
    for (let i = 0; i < torsoPoints.length; i++) {
      ...
      lines.push(
        `<line x1="${current.x}" y1="${current.y}" x2="${next.x}" y2="${next.y}" stroke="#ffffff" stroke-width="2" fill="none"/>`,
      );
    }
    ...
    if (morphs.muscleMorph > 0.3) {
      ...
      muscleLines.push(
        `<line x1="${this.centerX - 30}" y1="${this.centerY - 50}" x2="${this.centerX + 30}" y2="${this.centerY - 50}" stroke="#ffffff" stroke-width="1" fill="none" opacity="${muscleOpacity}"/>`,
      );
      muscleLines.push(
        `<line x1="${this.centerX - 25}" y1="${this.centerY + 20}" x2="${this.centerX + 25}" y2="${this.centerY + 20}" stroke="#ffffff" stroke-width="1" fill="none" opacity="${muscleOpacity}"/>`,
      );
    }
  ...
  async renderAllCombinations() {
    ...
    console.log(
      `🎯 Rendering ${totalCombinations} simplified SVG avatars (body fat + gender only)...`,
    );
    ...
    for (const sex of sexes) {
      for (const bodyFat of bodyFatValues) {
        ...
        try {
          ...
          console.error(
            `Error rendering ${this.generateFilename(params)}:`,
            error,
          );
        }
      }
    ...
    console.log(
      `✅ Completed rendering ${count} SVG avatars in ${totalTime.toFixed(1)}s`,
    );
