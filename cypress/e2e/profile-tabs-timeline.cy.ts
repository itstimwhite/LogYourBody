describe('Profile Screen v2 - Tabs & Timeline', () => {
  beforeEach(() => {
    ...
    cy.window().then((win) => {
      ...
      win.localStorage.setItem('logyourbody-user', JSON.stringify(testData.user));
      win.localStorage.setItem('logyourbody-metrics', JSON.stringify(testData.metrics));
    });
  ...
  describe('Tab Navigation', () => {
    ...
    it('should start with Avatar tab selected', () => {
      ...
      cy.get('[role="tab"]').last().should('have.attr', 'aria-selected', 'false');
    });
  ...
  describe('Timeline Slider', () => {
    ...
    it('should have proper accessibility labels', () => {
      ...
      cy.get('[role="slider"]').invoke('attr', 'aria-label').should('contain', 'Timeline slider');
    });
  });
  ...
  describe('Age Calculation', () => {
    ...
    it('should not show age when birthDate is missing', () => {
      ...
      cy.window().then((win) => {
        const user = { id: '1', name: 'Test User', gender: 'male', height: 180 };
  ...
  describe('Performance & Accessibility', () => {
    it('should complete tab switch in under 300ms', () => {
      ...
      cy.get('[role="tab"]').last().click();
      cy.get('[role="tab"]').last().should('have.attr', 'aria-selected', 'true').then(() => {
    ...
    it('should complete timeline navigation in under 200ms', () => {
      ...
      cy.get('[role="slider"]').click();
      cy.get('[role="slider"]').type('{rightarrow}');
      cy.contains('Entry 2 of 2').should('be.visible').then(() => {
        const duration = Date.now() - start;
