describe("RevenueCat Integration", () => {
  ...
  describe("RevenueCat Debug Card", () => {
    it('should show "Ready" status when VITE_REVENUECAT_PUBLIC_KEY is properly configured', () => {
      ...
      cy.get('[data-testid="settings-page"]', { timeout: 10000 }).should(
        "be.visible",
      );
      ...
      cy.get('[data-testid="revenuecat-debug-card"]').within(() => {
        ...
        cy.get('[data-testid="config-status-badge"]').should(
          "contain",
          "Configured",
        );
    ...
    it("should handle RevenueCat initialization and show customer info", () => {
      ...
      cy.get('[data-testid="revenuecat-debug-card"]').within(() => {
        ...
        cy.get('[data-testid="loading-status-badge"]').should(
          "contain",
          "Ready",
        );
        ...
        cy.get('[data-testid="restore-purchases-btn"]').should(
          "not.be.disabled",
        );
    ...
    it("should display test functionality correctly", () => {
      ...
      cy.get('[data-testid="revenuecat-debug-card"]').within(() => {
        ...
        cy.get('[data-testid="test-offerings-btn"]').should(
          "contain",
          "Loading",
        );

        // After test completes, should show result (assuming mock data)
        cy.get('[data-testid="test-offerings-btn"]', { timeout: 5000 }).should(
          "not.contain",
          "Loading",
        );
      });
