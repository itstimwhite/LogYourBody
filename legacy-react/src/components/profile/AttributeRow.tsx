...
export const AttributeRow = React.memo<AttributeRowProps>(
  function AttributeRow({ user, userAge, formattedHeight, className }) {
    ...
    const attributes: AttributeItem[] = [
      {
        ...
        accessibilityLabel: shouldShowAge
        ...
        {attributes.map((attr, index) => (
          <motion.div
            ...
            className={cn(
              ...
              attr.hidden && "pointer-events-none opacity-0",
            )}
          ...
          >
            {/* Label */}
            <div className={cn(tw.attributeLabel, "mb-1")}>{attr.label}</div>
...
export const useAttributeAnnouncement = (
  user: UserProfile,
  age: number,
  height: string,
) => {
