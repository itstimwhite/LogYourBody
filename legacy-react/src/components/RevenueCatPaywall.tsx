...
export function RevenueCatPaywall({
    ...
    try {
      ...
      if (hasProFeatures(updatedCustomerInfo)) {
        toast({
          ...
          description:
            "Welcome to LogYourBody Pro! Enjoy all premium features.",
        });
        onSuccess?.();
      } else {
  ...
  const defaultOffering =
    offerings.find((o) => o.identifier === "default") || offerings[0];

  if (!defaultOffering) {
    return (
      <div className="flex items-center justify-center p-8">
        <div className="text-center">
          <h3 className="mb-2 text-lg font-semibold">No Plans Available</h3>
          <p className="text-muted-foreground">
  ...
  const monthlyPackage = defaultOffering.availablePackages.find(
    (p) => p.identifier === "monthly" || p.rcBillingPeriod?.unit === "month",
  );

  const annualPackage = defaultOffering.availablePackages.find(
    (p) => p.identifier === "annual" || p.rcBillingPeriod?.unit === "year",
  );
  ...
  return (
    <div className="mx-auto max-w-md p-6">
      <div className="mb-8 text-center">
        ...
        <p className="text-muted-foreground">
          Unlock all features and get the most out of your body composition
          tracking
        </p>
      </div>

      {/* Billing Toggle */}
      <div className="mb-8 flex items-center justify-center gap-4" role="group">
        <span
          className={`text-sm font-medium ${selectedPlan === "monthly" ? "text-foreground" : "text-muted-foreground"}`}
        >
          Monthly
        </span>
        <Switch
        ...
        />
        <span
          className={`text-sm font-medium ${selectedPlan === "annual" ? "text-foreground" : "text-muted-foreground"}`}
        >
          Annual
        </span>
        {selectedPlan === "annual" && (
          <Badge className="ml-2 border-green-200 bg-green-100 text-green-800">
            Save {savingsPercent}%
          </Badge>
        )}
      </div>
          ...
          <div className="mt-4">
            <span className="text-3xl font-bold">
              ...
              {selectedPackage?.product.price ||
                (selectedPlan === "annual" ? "69.99" : "9.99")}
            </span>
