...
export const DatabaseStatusCard = React.memo<DatabaseStatusCardProps>(
    ...
    return (
      <SettingGroup title="Database & Storage" className={className}>
        ...
        <SettingItem
          ...
          subtitle={
            status.error ||
            `${status.latency ? `${status.latency}ms latency` : "Checking connection..."}`
          }
          icon={
          ...
          accessory={
            <div className="flex items-center gap-2">
              <div className={connectionInfo.color}>{connectionInfo.icon}</div>
              <motion.button
                onClick={handleRefresh}
                disabled={isRefreshing}
                className={cn(
                  "rounded-full p-1 text-muted-foreground transition-colors hover:text-foreground",
